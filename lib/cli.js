...
internals.traverse = function (paths, options) {
    ...
    if (options.preprocessor) {
        var preprocessor = options.preprocessor.split(':');
        var ext = preprocessor[0];
        var mod = preprocessor[1];
        extensions.push(ext);
        require(mod);
    }
    var ext_re = new RegExp('\\.(' + extensions.join('|') + ')$');
...
internals.options = function () {
    ...
    var args = {
        ...
        preprocessor: {
            short: 'P',
            type: 'string',
            description: '<ext>:<module> uses module to compile files with ext'
        },
        reporter: {
            short: 'r',
            type: 'string',
    ...
    var keys = ['coverage', 'colors', 'dry', 'environment', 'flat', 'grep', 'globals', 'timeout', 'parallel', 'reporter', 'threshold', 'preprocessor'];
